...
interface Props {
  onBack: () => void;
  viewUserId?: number; // если задан — просмотр чужого профиля (только для админа)
}
...
export default function ProfilePage({ onBack, viewUserId }: Props) {
  ...
  if (!profile) return (
    <div className="min-h-screen bg-[#36393f] flex items-center justify-center">
      <div className="text-[#ed4245] text-sm">Профиль не найден</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#36393f] text-white flex flex-col">
      ...
      <nav className="bg-[#2f3136] border-b border-[#202225] px-4 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <span className="text-white font-semibold text-sm">
          {isOwn ? "Мой кабинет" : `Профиль: ${profile.username}`}
        </span>
      </nav>
          ...
          <div className="px-5 pb-5">
            <div className="flex items-end gap-4 -mt-8 mb-4">
              <div className={`w-20 h-20 rounded-full border-4 border-[#2f3136] flex items-center justify-center text-white text-3xl font-bold flex-shrink-0 ${AVATAR_BG[profile.role]}`}>
                {avatarLetter(profile.full_name || profile.username)}
              </div>
              <div className="pb-1">
                <div className="text-white text-xl font-bold">{profile.full_name || profile.username}</div>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${profile.role === "admin" ? "bg-[#ed4245]/20 text-[#ed4245]" : profile.role === "teacher" ? "bg-[#3ba55c]/20 text-[#3ba55c]" : "bg-[#5865f2]/20 text-[#5865f2]"}`}>
          ...
          <div className="bg-[#2f3136] rounded-lg border border-[#202225] overflow-hidden">
            <div className="flex border-b border-[#202225]">
              <button onClick={() => setTab("info")}
                className={`flex-1 py-3 text-sm font-medium transition-colors flex items-center justify-center gap-2 ${tab === "info" ? "text-white border-b-2 border-[#5865f2]" : "text-[#8e9297] hover:text-[#dcddde]"}`}>
                <Icon name="Edit3" size={14} />
                Редактировать
              </button>
              <button onClick={() => setTab("password")}
                className={`flex-1 py-3 text-sm font-medium transition-colors flex items-center justify-center gap-2 ${tab === "password" ? "text-white border-b-2 border-[#5865f2]" : "text-[#8e9297] hover:text-[#dcddde]"}`}>
            ...
            <div className="p-5">
              ...
              {tab === "password" && (
                <div className="space-y-6">
                  {/* Своя смена (только если это свой профиль) */}
                  {isOwn && (
                    <form onSubmit={handleChangePw} className="space-y-4">
                      <h3 className="text-white font-medium text-sm">Сменить свой пароль</h3>
                      <div>
                        <label className="text-[#b9bbbe] text-xs font-semibold uppercase tracking-wide block mb-1.5">Текущий пароль</label>
                        <input type="password" value={pwForm.old_password} onChange={e => setPwForm(p => ({ ...p, old_password: e.target.value }))}
                          className="w-full bg-[#40444b] text-white placeholder-[#72767d] rounded px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-[#5865f2]"
                          placeholder="••••••••" required />
                      </div>
                      <div>
                        <label className="text-[#b9bbbe] text-xs font-semibold uppercase tracking-wide block mb-1.5">Новый пароль</label>
                      ...
                      <div>
                        <label className="text-[#b9bbbe] text-xs font-semibold uppercase tracking-wide block mb-1.5">Повторите пароль</label>
                  ...
                  {/* Смена пароля другому пользователю (только для админа) */}
                  {isAdmin && !isOwn && (
                  ...
                  {/* Если смотришь свой профиль будучи админом — оба блока */}
                  {isAdmin && isOwn && (
                    <p className="text-[#72767d] text-xs">Для смены пароля другим пользователям — откройте их профиль через раздел «Аккаунты» в панели администратора.</p>
                  )}
                </div>
              )}
            </div>
